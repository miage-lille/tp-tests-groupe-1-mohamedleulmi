...
describe('Webinar Routes E2E', () => {
  ...
  it('should return 400 if trying to reduce the number of seats', async () => {
    // ARRANGE
    const prisma = fixture.getPrismaClient();
    const server = fixture.getServer();
    ...
    // ASSERT
    expect(response.body).toEqual({
      error: 'You cannot reduce the number of seats',
    });
  });

  it('should return 400 if trying to set seats to a number greater than 1000', async () => {
    // ARRANGE
    const prisma = fixture.getPrismaClient();
    const server = fixture.getServer();
